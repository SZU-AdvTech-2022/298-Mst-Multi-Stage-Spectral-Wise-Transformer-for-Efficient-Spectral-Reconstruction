...
class TrainDataset(Dataset):
    def __init__(self, data_root, crop_size, arg=True, bgr2rgb=True, stride=8):
        ...
        hyper_data_path = f'{data_root}/GF-5_Spec_Train/'
        bgr_data_path = f'{data_root}/GF-5_RGB_Train/'
        ...
        hyper_list.sort()
        bgr_list.sort()
        print(f'len(hyper) of GF-5 dataset:{len(hyper_list)}')
        print(f'len(bgr) of GF-5 dataset:{len(bgr_list)}')
        for i in range(len(hyper_list)//10):
            ...
            mat.close()
            print(f'GF-5 scene {i} is loaded.')
...
class ValidDataset(Dataset):
    def __init__(self, data_root, bgr2rgb=True):
        self.hypers = []
        self.bgrs = []
        hyper_data_path = f'{data_root}/GF-5_Spec_Validation/'
        bgr_data_path = f'{data_root}/GF-5_RGB_Validation/'
        with open(f'{data_root}/split_txt/valid_list.txt', 'r') as fin:
            hyper_list = [line.replace('\n', '.mat') for line in fin]
            bgr_list = [line.replace('mat','jpg') for line in hyper_list]
        hyper_list.sort()
        bgr_list.sort()
        print(f'len(hyper_valid) of GF-5 dataset:{len(hyper_list)}')
        print(f'len(bgr_valid) of GF-5 dataset:{len(bgr_list)}')
